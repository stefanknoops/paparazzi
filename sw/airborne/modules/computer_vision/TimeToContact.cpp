...
{	
	...
	//	 prev_image = image;
	 ...
	 for (int i=0 ; i< (uflow.rows); i++){
		 ...
		 vectors_ptr[j + i * uflow.cols].pos.x = (uint32_t)(j * subpixel_factor);
		 vectors_ptr[j + i * uflow.cols].pos.y = (uint32_t)(i * subpixel_factor);
		 vectors_ptr[j + i * uflow.cols].flow_x = (float)(uflow.at<Vec2f>(i,j)[0]* subpixel_factor);
		 vectors_ptr[j + i * uflow.cols].flow_y = (float)(uflow.at<Vec2f>(i,j)[1] * subpixel_factor);
		 }
	 }
