...
struct image_t *farneback(struct image_t *img);
struct image_t *farneback(struct image_t *img)
{
    opt_flow = calcOpticalFlowFarneback(InputArray prev, InputArray next, InputOutputArray flow, double pyr_scale, int levels, int winsize, int iterations, int poly_n, double poly_sigma) {
    flow_im = flow;
